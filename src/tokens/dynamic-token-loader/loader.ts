...
export class DynamicTokenLoader {
  ...
  getCurrentTenant(): string | null {
    return this.tenantLoader.getCurrentTenant();
  }

  /**
   * Clear current tenant configuration
   */
  clearTenantConfiguration(): void {
    this.tenantLoader.clearTenant();
  }

  /**
   * Inject custom tokens
   */
  injectCustomTokens(tokens: Record<string, TokenValue>): void {
    // Apply custom tokens to the token system
    Object.entries(tokens).forEach(([path, value]) => {
      this.injectToken(path, value);
    });
  }
  ...
  getCachedTokens(tenantId: string): Record<string, TokenValue> | null {
    return this.tenantLoader.getCachedTokens(tenantId);
  }
   ...
   */
  private injectToken(path: string, value: TokenValue): void {
   ...
   */
  private validateConfig(config: DynamicTokenLoaderConfig): void {
