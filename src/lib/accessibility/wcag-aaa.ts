...
export const WCAG_AAA_STANDARDS = {
  NORMAL_TEXT: 7.0,     // 7:1 ratio for normal text
  LARGE_TEXT: 4.5,      // 4.5:1 ratio for large text (18pt+ or 14pt+ bold)
  GRAPHICAL: 3.0,       // 3:1 ratio for graphical objects and UI components
} as const;
...
export const focusManagement = {
   ...
   */
  getFocusRing: (color: string = '#1987ff'): string => {
    return `
...
export const colorContrast = {
   ...
   */
  getBestContrastColor: (backgroundColor: string, textColors: string[]): string => {
    if (textColors.length === 0) {
      throw new Error('No text colors provided');
    }
...
export const screenReader = {
   ...
   */
  getScreenReaderOnlyStyles: (): string => {
    return `
      position: absolute;
      width: 1px;
      height: 1px;
...
export const motionAccessibility = {
  ...
  getSafeAnimationDuration: (duration: number): number => {
    return motionAccessibility.prefersReducedMotion() ? 0 : duration;
  },
...
export const touchTarget = {
   ...
   */
  ensureMinimumSize: (width: number, height: number): { width: number; height: number } => {
    return {
      width: Math.max(width, touchTarget.MINIMUM_SIZE),
   ...
   */
  getTouchTargetStyles: (padding: number = 12): string => {
    return `
...
export const typographyAccessibility = {
   ...
   */
  getAccessibleFontSize: (baseFontSize: number, zoomLevel: number = 1): number => {
    return Math.max(baseFontSize * zoomLevel, 12); // Minimum 12px
  },
  ...
  getAccessibleLineHeight: (fontSize: number): number => {
    return Math.max(fontSize * 1.5, 24); // Minimum 1.5x line height
  },

  /**
   * Check if text size qualifies as "large text" per WCAG
   */
  isLargeText: (fontSize: number, fontWeight: number = 400): boolean => {
