...
import { generateRTLTokens,
  RTL_COMPONENT_UTILITIES,
  NORWEGIAN_RTL_TESTING, } from '../tokens/rtl-design-tokens';

// RTL Test Suite Interface
export interface RTLTestSuite { testName: string;
...
export interface RTLTestData { text: string;
  numbers: string;
  mixedContent: string;
...
export interface RTLExpectedResults { textAlignment: 'left' | 'right' | 'center';
  direction: TextDirection;
  marginStart: string;
  marginEnd: string;
  paddingStart: string;
...
// RTL Test Results
export interface RTLTestResults { passed: boolean;
  errors: string[];
  warnings: string[];
  performance: { renderTime: number;
    memoryUsage: number; };
  accessibility: { screenReaderCompatible: boolean;
    keyboardNavigable: boolean; }; }

// Arabic Language Test Suite
export const ARABIC_TEST_SUITE: RTLTestSuite = { testName: 'Arabic RTL Support',
  locale: 'ar',
  direction: 'rtl',
  testData: { text: 'مرحبا بكم في النرويج - أهلا وسهلا',
    numbers: '١٢٣٤٥٦٧٨٩٠',
  ...
  expectedResults: { textAlignment: 'right',
    direction: 'rtl',
...
export const HEBREW_TEST_SUITE: RTLTestSuite = { testName: 'Hebrew RTL Support',
  locale: 'he',
  direction: 'rtl',
  testData: { text: 'ברוכים הבאים לנורווגיה - שלום',
    numbers: '1234567890',
...
export const NORWEGIAN_RTL_TEST_SUITE: RTLTestSuite = { testName: 'Norwegian in RTL Mode (Accessibility Test)',
  locale: 'nb-NO',
  direction: 'rtl',
  testData: { text: 'Velkommen til Norge - RTL Test',
    numbers: '12345678901',
...
export class RTLComponentTester { private testResults: Map<string, RTLTestResults> = new Map();

  // Test a component with specific RTL configuration
  testComponent(componentElement: HTMLElement, testSuite: RTLTestSuite): RTLTestResults { const startTime = performance.now();
    const errors: string[] = [];
    const warnings: string[] = [];

    try { // Set up RTL environment
      ...
      if (testSuite.locale === 'nb-NO' || testSuite.locale === 'nn-NO') { this.testNorwegianRTLCompliance(componentElement, testSuite, errors, warnings); }

      // Test accessibility in RTL mode
      const accessibility = this.testRTLAccessibility(componentElement);

      const endTime = performance.now();
      const results: RTLTestResults = { passed: errors.length === 0,
        errors,
        warnings,
        performance: { renderTime: endTime - startTime,
          memoryUsage: this.estimateMemoryUsage(), },
        accessibility, };

      this.testResults.set(testSuite.testName, results);
      return results; } catch (error) { errors.push(`RTL test failed: ${error}`);
      return { passed: false,
        errors,
        warnings,
  ...
  private setupRTLEnvironment(element: HTMLElement, testSuite: RTLTestSuite): void { element.dir = testSuite.direction;
    ...
    // Insert test content
    if (element.textContent !== null) { element.textContent = testSuite.testData.text; } }
  ...
  private testDirectionHandling(
    ...
    errors: string[],
    warnings: string[]
  ): void { const computedStyle = global.window.getComputedStyle(element);
    const actualDirection = computedStyle.direction;

    if (actualDirection !== testSuite.expectedResults.direction) { errors.push(
        `Direction mismatch: expected ${testSuite.expectedResults.direction}, got ${actualDirection}`
      ); }

    // Check for dir attribute
    if (element.dir !== testSuite.direction) { warnings.push(`Element dir attribute should be "${testSuite.direction}"`); } }
  ...
  private testSpacingProperties(
    ...
    if (testSuite.expectedResults.hasLogicalProperties && !hasLogicalProperties) { warnings.push(
    ...
    if (testSuite.direction === 'rtl') { const marginLeft = computedStyle.marginLeft;
      const marginRight = computedStyle.marginRight;

      if (marginLeft !== '0px' && marginRight !== '0px') { warnings.push('Avoid using both margin-left and margin-right in RTL mode'); } } }
  ...
  private testTextAlignment(
    ...
    if (testSuite.direction === 'rtl') { if (textAlign !== 'right' && textAlign !== 'start') { warnings.push(`Text alignment "${textAlign}" may not be optimal for RTL`); } } }
  ...
  private testBiDiSupport(
    ...
    if (testSuite.expectedResults.biDiIsolation) { if (unicodeBidi !== 'isolate' && unicodeBidi !== 'isolate-override') { warnings.push('Consider using unicode-bidi: isolate for better BiDi support'); } }

    // Test mixed content handling
    if (testSuite.testData.mixedContent) { const hasProperBiDiMarks = this.checkBiDiMarks(testSuite.testData.mixedContent);
      if (!hasProperBiDiMarks) { warnings.push('Mixed content may benefit from BiDi control characters'); } } }
  ...
  private testNorwegianRTLCompliance(
    ...
    errors: string[],
    warnings: string[]
  ): void { // Test Norwegian personal number formatting in RTL
    if (testSuite.testData.norwegianContent?.includes('12345678901')) { const content = element.textContent || '';
      if (!content.includes('123456 78901') && !content.includes('12345678901')) { warnings.push('Norwegian personal numbers should maintain proper formatting in RTL'); } }

    // Test government service terms in RTL
    if (testSuite.testData.governmentTerms) { testSuite.testData.governmentTerms.forEach(term => { if (term.includes('Altinn') || term.includes('ID-porten')) { // These should maintain LTR orientation even in RTL context
          warnings.push(`Government service names like "${term}" may need LTR override`); } }); } }

  // Test RTL accessibility
  private testRTLAccessibility(element: HTMLElement): { screenReaderCompatible: boolean;
    keyboardNavigable: boolean; } { // Check for proper ARIA labels in RTL context
    const hasAriaLabel =
    ...
    return { screenReaderCompatible: hasAriaLabel,
      keyboardNavigable: isKeyboardNavigable, }; }

  // Check for BiDi control characters
  private checkBiDiMarks(text: string): boolean { const bidiMarks = [
  ...
  private estimateMemoryUsage(): number { // Mock implementation - in real scenario would use performance.measureUserAgentSpecificMemory
    return Math.random() * 1000; }

  // Get all test results
  getTestResults(): Map<string, RTLTestResults> { return this.testResults; }

  // Generate RTL test report
  generateTestReport(): string { let report = '# RTL Testing Report\n\n';

    this.testResults.forEach((results, testName) => { report += `## ${testName}\n`;
      ...
      if (results.warnings.length > 0) { report += '**Warnings:**\n';
...
export const runAllRTLTests = (componentElement: HTMLElement): Map<string, RTLTestResults> => { const tester = new RTLComponentTester();
