...
export interface RTLDesignTokens { direction: TextDirection;
...
export const RTL_SPACING_TOKENS = { // Logical properties for RTL support
  marginStart: { ltr: 'margin-left',
    rtl: 'margin-right',
    token: 'var(--spacing-margin-start)', },
  marginEnd: { ltr: 'margin-right',
    rtl: 'margin-left',
    token: 'var(--spacing-margin-end)', },
  paddingStart: { ltr: 'padding-left',
    rtl: 'padding-right',
    token: 'var(--spacing-padding-start)', },
  paddingEnd: { ltr: 'padding-right',
    rtl: 'padding-left',
    token: 'var(--spacing-padding-end)', },
  borderStartWidth: { ltr: 'border-left-width',
    rtl: 'border-right-width',
    token: 'var(--border-start-width)', },
  borderEndWidth: { ltr: 'border-right-width',
    rtl: 'border-left-width',
    token: 'var(--border-end-width)', }, };

// RTL-aware positioning tokens
export const RTL_POSITION_TOKENS = { start: { ltr: 'left',
    rtl: 'right',
    token: 'var(--position-start)', },
  end: { ltr: 'right',
    rtl: 'left',
    token: 'var(--position-end)', },
  textAlign: { start: { ltr: 'left',
      rtl: 'right', },
    end: { ltr: 'right',
      rtl: 'left', }, }, };

// RTL-aware typography tokens
export const RTL_TYPOGRAPHY_TOKENS = { direction: { ltr: 'ltr',
    rtl: 'rtl',
...
export const generateRTLTokens = (direction: TextDirection): RTLDesignTokens => { return { direction,
    spacing: { marginStart:
    ...
    positioning: { left: direction === 'rtl' ? 'var(--position-right)' : 'var(--position-left)',
    ...
    layout: {},
    typography: { direction: direction,
      textAlign: direction === 'rtl' ? 'right' : 'left',
      unicodeBidi: 'isolate', }, }; };
...
export const RTL_CSS_PROPERTIES = `
  ...
  /* RTL overrides */
  [dir="rtl"] { --text-direction: rtl;
    --position-start: auto;
    --position-end: 0; }
  
  /* Logical property support fallbacks */
  .margin-start { margin-left: var(--spacing-margin-start);
    margin-inline-start: var(--spacing-margin-start); }
  .margin-end { margin-right: var(--spacing-margin-end);
    margin-inline-end: var(--spacing-margin-end); }
  .padding-start { padding-left: var(--spacing-padding-start);
    padding-inline-start: var(--spacing-padding-start); }
  .padding-end { padding-right: var(--spacing-padding-end);
    padding-inline-end: var(--spacing-padding-end); }
  
  [dir="rtl"] .margin-start { margin-left: auto;
    margin-right: var(--spacing-margin-start); }
  [dir="rtl"] .margin-end { margin-right: auto;
    margin-left: var(--spacing-margin-end); }
  [dir="rtl"] .padding-start { padding-left: auto;
    padding-right: var(--spacing-padding-start); }
  [dir="rtl"] .padding-end { padding-right: auto;
    padding-left: var(--spacing-padding-end); }
`;

// Norwegian RTL testing utilities
export const NORWEGIAN_RTL_TESTING = { // Test Norwegian interface in RTL mode for accessibility testing
  testStrings: { 'nb-NO': [
  ...
  mixedLanguageTests: { norwegianArabic: ['Velkommen مرحبا til Norge', 'Fødselsnummer رقم الهوية: 12345678901'],
    norwegianHebrew: ['Velkommen ברוכים הבאים til Norge', 'Fødselsnummer מספר זהות: 12345678901'], },

  // RTL form field testing
  formFieldTests: { labels: { ltr: 'Fødselsnummer:',
      rtl: ':رقم الهوية', },
    placeholders: { ltr: 'Skriv inn fødselsnummer',
      rtl: 'أدخل رقم الهوية', }, }, };

// RTL component utilities
export const RTL_COMPONENT_UTILITIES = { // Get appropriate margin/padding for RTL
  getSpacing: (property: 'margin' | 'padding', side: 'start' | 'end', value: string) => { return `var(--spacing-${property}-${side})`; },

  // Get appropriate positioning for RTL
  getPosition: (side: 'start' | 'end') => { return `var(--position-${side})`; },

  // Generate RTL-aware CSS
  generateRTLCSS: (styles: Record<string, string>, direction: TextDirection) => { const rtlStyles: Record<string, string> = {};

    Object.entries(styles).forEach(([property, value]) => { if (property.includes('left') && direction === 'rtl') { rtlStyles[property.replace('left', 'right')] = value; } else if (property.includes('right') && direction === 'rtl') { rtlStyles[property.replace('right', 'left')] = value; } else { rtlStyles[property] = value; } });

    return rtlStyles; },

  // Validate RTL support in component
  validateRTLSupport: (component: HTMLElement): boolean => { const computedStyle = window.getComputedStyle(component);
...
export const DEFAULT_RTL_CONFIG = { supportedDirections: ['ltr', 'rtl'] as TextDirection[],
  rtlLocales: ['ar', 'he'],
