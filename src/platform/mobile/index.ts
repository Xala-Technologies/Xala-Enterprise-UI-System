...
export const mobileUtils = { // Touch target validation (WCAG 2.2 AA - minimum 44px)
  validateTouchTarget: (size: number): boolean => size >= 44,

  // Safe area utilities
  getSafeAreaInsets: () => ({ top: 'env(safe-area-inset-top)',
    bottom: 'env(safe-area-inset-bottom)',
    left: 'env(safe-area-inset-left)',
    right: 'env(safe-area-inset-right)', }),

  // Norwegian mobile compliance
  norwegianCompliance: { // DigDir mobile guidelines
    digdir: { minimumTouchTarget: 44, // px
      recommendedTouchTarget: 48, // px
      maximumTouchTarget: 64, // px
      tapAreaSpacing: 8, // px minimum between targets },

    // NSM mobile security
    nsm: { classificationIndicators: true,
      auditLogging: true,
      emergencyAccess: true,
      secureDataHandling: true, },

    // WCAG 2.2 AA mobile
    wcag: { minimumContrastRatio: 4.5,
  ...
  // Gesture utilities
  gestures: { swipeThreshold: 50, // px
    swipeVelocityThreshold: 0.5, // px/ms
    tapThreshold: 10, // px
    longPressDelay: 500, // ms }, };

// Norwegian mobile constants
export const NORWEGIAN_MOBILE_CONSTANTS = { municipalities: { // Major Norwegian municipalities with mobile optimizations
    oslo: '0301',
    bergen: '1201',
    trondheim: '1601',
    stavanger: '1103',
