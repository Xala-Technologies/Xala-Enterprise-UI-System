...
const getToastStyles = (props: ToastProps): React.CSSProperties => { const { variant = 'info', position = 'bottom-right', norwegian } = props;
...
const getPositionStyles = (position: string): React.CSSProperties => { const positions = { 'top-left': { top: 'var(--spacing-4)',
      left: 'var(--spacing-4)', },
    'top-center': { top: 'var(--spacing-4)',
      left: '50%',
      transform: 'translateX(-50%)', },
    'top-right': { top: 'var(--spacing-4)',
      right: 'var(--spacing-4)', },
    'bottom-left': { bottom: 'var(--spacing-4)',
      left: 'var(--spacing-4)', },
    'bottom-center': { bottom: 'var(--spacing-4)',
      left: '50%',
      transform: 'translateX(-50%)', },
    'bottom-right': { bottom: 'var(--spacing-4)',
      right: 'var(--spacing-4)', }, };
  return positions[position as keyof typeof positions] || positions['bottom-right']; };

// Get variant-based styles
const getVariantStyles = (variant: string): React.CSSProperties => { const variants = { info: { backgroundColor: 'var(--color-blue-600)',
      borderColor: 'var(--color-blue-700)',
      color: 'var(--color-white)', },
    success: { backgroundColor: 'var(--color-green-600)',
      borderColor: 'var(--color-green-700)',
      color: 'var(--color-white)', },
    warning: { backgroundColor: 'var(--color-orange-600)',
      borderColor: 'var(--color-orange-700)',
      color: 'var(--color-white)', },
    error: { backgroundColor: 'var(--color-red-600)',
...
const ToastIcon = ({ _variant, icon }: { variant: string; icon?: React.ReactNode }): React.ReactElement => { if (icon) { return (
      <span
...
// Toast action button
const ToastActionButton = ({ action }: { action: { label: string; handler: () => void } }): React.ReactElement => { return (
    <button
      ...
      onClick={e => { e.stopPropagation();
        action.handler(); }}
      onMouseEnter={e => { (e.target as HTMLElement).style.backgroundColor = 'var(--color-white-alpha-30)'; }}
      onMouseLeave={e => { (e.target as HTMLElement).style.backgroundColor = 'var(--color-white-alpha-20)'; }}
...
const PriorityIndicator = ({ priority }: { priority?: string }): React.ReactElement => { return (
    <span
      style={{ fontSize: 'var(--font-size-xs)',
...
export const Toast = React.forwardRef<HTMLDivElement, ToastProps>((props, ref): React.ReactElement => { const { isOpen = true,
    variant = 'info',
    title,
    titleKey,
    message,
  ...
  useEffect(() => { if (isOpen) { setIsVisible(true);
      onOpen?.(); } else { setIsVisible(false); } }, [isOpen, onOpen]);

  useEffect(() => { if (!isVisible || persistent || isPaused) return;

    const timer = setTimeout(() => { handleClose(); }, duration);

    return () => clearTimeout(timer); }, [isVisible, duration, persistent, isPaused]);

  const handleClose = (): void => { setIsVisible(false);
    onClose?.(); };

  const handleMouseEnter = (): void => { if (pauseOnHover) { setIsPaused(true); } };

  const handleMouseLeave = (): void => { if (pauseOnHover) { setIsPaused(false); } };

  const combinedStyles = { ...getToastStyles(props),
    ...style, };

  const getToastRole = (): string => { return variant === 'error' || variant === 'warning' ? 'alert' : 'status'; };

  const getAriaLive = (): 'polite' | 'assertive' | 'off' => { if (variant === 'error') return 'assertive';
