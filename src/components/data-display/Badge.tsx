...
export function Badge({ children,
  ...
  className = '',
  testId,
  ...props }: BadgeProps): React.ReactElement { const { t } = useLocalization();

  // Format count display
  const displayCount = React.useMemo((): React.ReactNode => { if (typeof count !== 'number') { return children; }
    if (count === 0 && !showZero) { return null; }
    return count > maxCount ? `${maxCount}+` : count.toString(); }, [count, maxCount, showZero, children]);

  // Build CSS classes using design tokens
  const badgeClasses = React.useMemo((): string => { const classes = ['badge'];
    ...
    // Position classes
    if (position !== 'inline') { classes.push('badge--positioned');
    ...
    if (classification) { classes.push(`badge--classification-${classification}`); }

    // Priority classes
    if (priority) { classes.push(`badge--priority-${priority}`); }

    // Custom classes
    if (className) { classes.push(className); }

    return classes.join(' '); }, [variant, size, shape, position, pulse, dot, classification, priority, className]);

  // Accessibility props
  const accessibilityProps = React.useMemo((): React.ReactElement => { return (
...
const PriorityIndicator: React.FC<{ priority: string }> = ({ priority }): React.ReactElement => { return (
